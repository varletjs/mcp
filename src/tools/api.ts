...
export async function registerApiTools (server: McpServer) {
  server.tool(
    ...
    {
      version: z.string().default('latest').describe('The version of Varlet to retrieve API types for, e.g., "latest" or "3.0.0"'),
    },
    async ({ version }) => {
  ...
  server.tool(
    ...
    {
      componentName: z.string().describe('The name of a Varlet component, available options: Button, Card, Cell, Dialog, Input, etc.'),
      version: z.string().default('latest').describe('The version of Varlet to retrieve API types for, e.g., "latest" or "3.0.0"'),
    },
  ...
  server.tool(
    ...
    {
      directiveName: z.string().describe('The name of a Varlet directive, e.g., "v-ripple" or "ripple"'),
      version: z.string().default('latest').describe('The version of Varlet to retrieve API types for, e.g., "latest" or "3.0.0"'),
    },
  ...
  server.tool(
    ...
    {
      version: z.string().default('latest').describe('The version of Varlet to retrieve components for'),
    },
    async ({ version }) => {
      const components = [
        'Button', 'Card', 'Cell', 'Checkbox', 'Dialog', 'Divider',
        'Fab', 'Form', 'Icon', 'Image', 'Input', 'List', 'Loading',
        'Menu', 'Overlay', 'Picker', 'Popup', 'Progress', 'Radio',
        'Rate', 'Select', 'Slider', 'Snackbar', 'Step', 'Sticky',
        'Switch', 'Tab', 'Table', 'Tooltip', 'Uploader'
      ]
