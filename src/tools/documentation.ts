...
export async function registerDocumentationTools (server: McpServer) {
  ...
  server.tool(
    ...
    'Get detailed information about how to install Varlet in a variety of environments.',
    {
  ...
  server.tool(
    'get_varlet_exports',
    'Get a list of exports from the @varlet/ui npm package',
    documentation.getVarletExports,
  )

  server.tool(
    'get_frequently_asked_questions',
    'Get a list of frequently asked questions about Varlet.',
    documentation.getFrequentlyAskedQuestions,
  )

  server.tool(
    'get_release_notes_by_version',
    'Get release notes for one or more versions of Varlet.',
    {
      version: z.string().describe('One or more Varlet versions for which to get the release notes.').default('latest'),
    },
  ...
  server.tool(
    ...
    'Get example code and playground links for Varlet components.',
    {
      component: z.string().describe('The component name to get examples for').optional(),
    },
    documentation.getPlaygroundExamples,
  )
}
